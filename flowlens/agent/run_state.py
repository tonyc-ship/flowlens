"""Structured per-run state for the generic browser agent."""
    ...
    def render_working_memory(self, *, max_recent_events: int = 10, max_evidence: int = 8) -> str:
        ...
        lines.extend(
            [
                "",
                "# Retrieval",
                "- Use update_task_plan to keep a live checklist for complex tasks.",
                '- Use read_run_state(section="evidence" | "artifacts" | "plan" | "working_memory") to revisit earlier findings.',
                '- Use read_saved_artifact(path="...") when you need the full saved JSON/text for an older result.',
                "",
            ]
        )
        return "\n".join(lines).rstrip() + "\n"
