"""Generic CDP diagnostics used by the FlowLens desktop runtime."""
...
FLOWLENS_TITLE_PREFIX = "🟢 FlowLens"

TEST_PAGE_HTML = """<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <title>FlowLens Primitive Test</title>
  <style>
...
<body>
  <h1>FlowLens Primitive Test</h1>
  <p>This page is created by the FlowLens desktop runtime diagnostic.</p>
  <button id="click-target" onclick="document.body.dataset.clicked='yes'; this.textContent='Clicked by FlowLens';">Click me</button>
...
def default_screenshot_path() -> Path:
    ...
    return Path(tempfile.gettempdir()) / "flowlens" / f"controlled_tab_{stamp}.png"
...
async def run_controlled_tab_diagnostic(config: ControlledTabConfig | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        marked_title = await page.set_title_prefix(FLOWLENS_TITLE_PREFIX)
        ...
        return {
            ...
            "reason": "Created and exercised a marked FlowLens-controlled Chrome tab.",
            "endpoint": endpoint,
...
async def exercise_typing(page: CDPPage) -> tuple[bool, bool]:
    ...
    if not isinstance(input_state.value, dict):
        return False, False
    return input_state.value.get("value") == "flowlens", input_state.value.get("enterPressed") == "yes"
...
def print_controlled_tab_human(result: dict[str, Any]) -> None:
    ...
    print(f"FlowLens controlled-tab status: {result['status']}")
...
def parse_controlled_tab_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create and verify a FlowLens-controlled Chrome CDP tab.")
