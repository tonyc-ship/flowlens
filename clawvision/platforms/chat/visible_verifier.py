...
class ChatbotWindowState(Protocol):
    site: Any
    planned_bounds: dict[str, int]
    status: str
    error: str
    visible_screenshots: list[Path]
...
class ChatbotVisibleVerifier:
    ...
    def _match_visible_windows(self, windows: list[ChatbotWindowState]) -> list[tuple[ChatbotWindowState, object | None]]:
        visible_windows = [
            window
    ...
    @staticmethod
    def _apply_status_label(cw: ChatbotWindowState, label: str) -> None:
        if label == "GENERATING":
    ...
    def verify(
        self,
        windows: list[ChatbotWindowState],
        question: str,
        *,
        stage: str,
        record_event=None,
