...
if ! command -v "$PNPM_BIN" >/dev/null 2>&1; then
  echo "pnpm is required to build the FlowLens app. Install it with: corepack enable pnpm" >&2
  exit 1
fi
...
for legacy_path in "/Applications/FlowLens Prototype.app" "/Applications/FlowLens Prototype.app" "/Applications/FlowLens Desktop.app"; do
