...
try {
  ...
  console.log('-'.repeat(70));
  await page.goto('https://www.xiaohongshu.com/', { waitUntil: 'networkidle' }).catch(e => {
    console.log('页面导航时出错（但继续）:', e.message);
  });
  ...
  await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(e => {
    console.log('搜索导航时出错（但继续）:', e.message);
  });
