...
#### 路径 A：AI 图文生成（推荐，baoyu-xhs-images）

适合需要 AI 绘制风格插图、视觉表现力强的内容：

```bash
# 根据风格标签选择 preset
/baoyu-xhs-images --preset <preset_name>
[粘贴第二步生成的内容结构]

# 常用 preset（与 analyze_accounts 输出对应）
# 干货/知识类  → --preset knowledge-card
# 情感/故事类  → --preset cozy-story
# 种草/产品类  → --preset product-review
# 教程/步骤类  → --preset tutorial
# 避坑/清单类  → --preset warning 或 checklist
# 留学/学习类  → --preset study-guide
```

生成结果：`xhs-images/<slug>/` 目录下 `01-cover.png` + `02..NN-content.png` + 结尾图

> 参数映射规则（analyze_accounts → baoyu-xhs-images）见 `agents/xhs-auto-agent.md` 第 5.1 节

---

#### 路径 B：本地 CSS 渲染（render_xhs.py）

适合需要精确排版控制、纯文字卡片、快速生成的场景：
