...
class LocalPublisher:
    ...
    def init_client(self):
        ...
        try:
            ...
            raise ImportError("缺少 xhs 库，请运行: pip install xhs")

        # external_sign 仅用于 web 端 GET 类接口（如 get_self_info）。
        # 旧 MD5 算法已被小红书风控识别，改为返回空 headers（不注入错误签名）。
        # 发布流程走 is_creator=True 路径，不经过此函数，不受影响。
...
'''
    ...
    # 加载 Cookie（多账号模式优先，回退到单账号 .env）
    account_name = None
    if _ACCOUNT_MANAGER_AVAILABLE:
        try:
            cookie, account_name = get_next_account(fallback_to_env=True)
            if account_name != "__env__":
                print(f"[账号] 使用：{account_name}")
        except AllAccountsExpiredError as e:
            print(f"❌ 错误：{e}")
            sys.exit(1)
    else:
        cookie = load_cookie()
    ...
    try:
        ...
        # 发布成功后更新轮询时间戳
        if _ACCOUNT_MANAGER_AVAILABLE and account_name and account_name != "__env__":
            mark_account_used(account_name)
    except Exception as e:
        sys.exit(1)
