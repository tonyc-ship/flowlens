...
`app/` is the active FlowLens desktop application. It is a Tauri 2 app with a Vite/TypeScript frontend, a Rust native shell, and a long-lived Python `flowlens.runtime` sidecar.
...
Python flowlens.runtime sidecar
        ↓
flowlens.cdp browser backend / task runtime
        ↓
User's existing Google Chrome profile
```

The app currently uses Chrome's remote-debugging / Chrome DevTools Protocol path to create a clearly marked `🟢 FlowLens` Chrome tab. Generic CDP discovery, target-listing, controlled-tab, and page primitives live in `flowlens.cdp`; Xiaohongshu CDP probe logic lives in `flowlens.platforms.xhs.cdp_diagnostics`. `app/prototype/` is deprecated and points to maintained wrappers under `scripts/diagnostics/`.
...
- Bundle identifier: `com.tonycship.flowlens`
...
1. FlowLens opens `chrome://inspect/#remote-debugging` in Google Chrome.
2. The user approves/enables Chrome remote debugging.
3. FlowLens discovers the local CDP endpoint for the existing Chrome profile.
4. FlowLens creates a clearly marked `🟢 FlowLens` tab.
...
```bash
FLOWLENS_PYTHON=/path/to/python3              # Python used for source-tree sidecar
FLOWLENS_DESKTOP_RUNTIME_PYTHON=/path/python  # Explicit sidecar Python override
FLOWLENS_DESKTOP_USE_UV_FOR_CDP=0             # Do not use uv for CDP diagnostic scripts
FLOWLENS_REPO_ROOT=/path/to/flowlens             # Override repository root for sidecar scripts
```
...
open "src-tauri/target/release/bundle/macos/FlowLens.app"
```
...
```text
FlowLens.app
  Contents/MacOS/FlowLens                 # Tauri/Rust binary
  Contents/Resources/frontend          # Built WebView assets
  Contents/Resources/flowlens-runtime/    # Bundled Python runtime + FlowLens package
```
