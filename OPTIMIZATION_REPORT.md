...
| 维度 | Browser-Use | Skyvern | Understudy | **SocAI** |
|------|-------------|---------|------------|----------------|
...
**对 SocAI 的价值**：XHS 有大量自定义组件，纯 DOM 树缺少语义，纯无障碍树缺少布局信息。三树融合后 LLM 对元素的理解准确率会显著提升，减少错误点击。
...
精准控制 LLM 对错误的关注度，避免错误信息占据过多 context window，非常适合 SocAI 的长任务场景。

#### ⑥ 消息历史压缩（MessageManager）

当对话历史过长时，用一个 LLM 调用把旧消息总结压缩（默认 40,000 字符触发，保留最近 6 条，旧的压缩到 6,000 字以内，每 15 步触发一次）。对 SocAI 的长时间研究任务很有参考价值。
...
**对 SocAI 的价值**：XHS 的笔记详情弹窗、标签下拉、搜索建议每次打开后都需要等待并重新扫描全量 DOM。改用增量提取后，只抓新出现的元素，速度预计提升 30-50%。
...
SocAI 已有 XHS 特定的反爬状态检测，但没有系统化分类。结构化失败类型能让重试策略更精准（比如 ANTI_BOT 触发时降速，LLM_ERROR 时切换模型）。
...
防止 LLM 过早声明研究任务完成，适用于 SocAI 的质量保证环节。
...
对 SocAI 的价值：DOM提取 → OCR → Vision API 这三条路径可以用同样的机制实现自动降级，不需要硬编码 fallback 逻辑。
...
放入对应目录自动加载。参考这种机制，可以让 SocAI 的平台知识库更易维护和扩展。

#### ⑤ 工作流结晶化（Workflow Crystallization）

系统自动分析历史对话，识别重复工作模式，自动合成可复用技能。对 SocAI 的启示：从历史成功任务中自动提炼 XHS 研究最佳实践，沉淀为下次任务的先验知识。

---

## 四、SocAI 的核心护城河（不要丢）

对比三个项目，SocAI 有一些他们完全没有的能力：
...
| Playwright 封装 | 不需要 | SocAI 的 Chrome 扩展方案复用用户登录状态，比 Playwright 更适合 XHS |
...
1. **Long URL Hash 压缩**
   - 改造 `socai/core/bridge.py` 或 DOM 序列化层
   - XHS 图片/CDN URL 哈希替换
   - 预计 prompt token 减少 20%，改动 <50 行

2. **失败类型分类器**
   - 在 `socai/core/` 下新建 `failure_classifier.py`
...
6. **Paint Order 过滤 + 三树融合**
   - 扩展 `socai/core/bridge.py` 的 CDP 调用
...
**对 SocAI 的价值**：XHS 任务中 `截图 + DOM提取 + 图片OCR` 完全可并发，目前是串行。改造后单卡片处理速度预计提升 40%+。
...
**对 SocAI 的价值**：SocAI 的 capability catalog 已经有类似分层概念。将 `lite` 能力始终加载，`deep` 能力（视频转录、详细图片分析等）延迟加载，可大幅缩小初始系统提示。
...
**对 SocAI 的价值**：XHS 批量研究任务的笔记内容、OCR 文本、图片描述可能很长，超出预算时存盘，让 LLM 按需读取，而非塞满 context window。
...
**对 SocAI 的价值**：XHS 反爬触发时，当前实现是直接失败。用 isWithheld 模式改造后，可以先降速重试、换策略，对上层任务透明，提升任务完成率。

#### ⑤ Hook 生命周期标准化（20+ 事件）

参考 Claude Code 的完整 Hook 事件体系，为 SocAI 定义标准生命周期：

```python
# 建议 SocAI 标准化的 Hook 事件
HOOK_EVENTS = {
    # 工具层
...
`PreToolUse` hook 输出支持 `updated_input`（修改工具参数）和 `permission_decision`（覆盖权限），这是 SocAI 目前没有的。

#### ⑥ JSONL 会话存储 + 断点续跑

Claude Code 的 JSONL 格式非常适合 SocAI 的任务记录：
...
**对 SocAI 的价值**：当前 SocAI 的任务运行输出是 HTML 报告 + GIF，不支持恢复中断的任务。JSONL 格式可以让长时间 XHS 研究任务支持断点续跑。

### 短期可落地（直接加入上面的优先级路线图）

| 优化点 | 落地位置 | 预期收益 |
|--------|----------|----------|
| 工具并发批处理 | `socai/core/bridge.py` | 单卡片处理速度 +40% |
| 工具结果磁盘持久化 | `socai/core/` 新建 `result_budget.py` | 长任务 context 可控 |
| isWithheld 错误自愈 | `socai/workflows/xhs/` | 任务完成率提升 |
| JSONL 会话格式 | `socai/core/recorder.py` 扩展 | 支持断点续跑 |
...
- **SocAI**：XHS 领域知识最深、本地化能力最强、反爬策略最实战——这是四个项目都没有的护城河，应在此方向持续加深，同时按优先级引入上述核心技术点
...
12. **🆕 Chrome 扩展不会自动重载**
    ...
    - 修复方向：agent 启动时调一次 `python -m socai extension reload`，或者在 `bridge.start()` 里加一个 file watcher。
...
- `socai/extension_ops.py` 里已经有 `extension reload` 的实现，agent loop 启动时调一次即可。
...
对 SocAI 本地模型路径，这个理念特别契合，因为 **prefill 是本地模型的唯一瓶颈**：
...
在本轮改造后，SocAI 已经补上了一个关键短板：**运行态不再只存在于聊天上下文里**。现在已经有：
...
- SocAI 现在还做不到精确回放某一轮当时的“原始上下文视图”，只能重建一个近似投影。

**G2. 还没有基于相关性的动态记忆检索器**
- 当前 `working_memory` 是从 plan / evidence / recent events 规则化渲染出来的，不是根据“当前问题”动态挑选最相关历史。
- Claude Code 更接近“按任务目标选择需要拉回哪些记忆块”，而不是统一模板式注入。
- SocAI 下一步需要的是：按当前 turn 的目标、待回答问题、正在使用的工具，对 `run_state` 做 relevance-based retrieval，而不是每次都塞固定摘要块。
...
- 这决定了上下文是否能长期稳定。SocAI 现在只是“多了一个可回读的后备仓库”，还不是“默认引用式上下文管理”。
