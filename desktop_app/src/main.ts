...
function render() {
  ...
  app.innerHTML = `
    <main class="shell">
      <header class="topbar">
        <div class="brand">
          <img class="brand-mark" src="${appIcon}" alt="SocAI app icon" />
          <span>SocAI</span>
        </div>
...
function renderWeChatMonitor(task: TaskStub | null): string {
  ...
  const caution = task?.controlActive
    ? `<div class="control-banner">SocAI is controlling WeChat. Do not fight for the mouse or keyboard.</div>`
    : "";
