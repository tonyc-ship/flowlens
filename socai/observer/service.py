...
class ObserverConfig:
    ...
    def from_env(cls) -> "ObserverConfig":
        load_runtime_env()
        return cls(
            check_interval=max(1, int(os.environ.get("SOCAI_OBSERVER_CHECK_INTERVAL", "5"))),
            force_capture_interval=max(
                30, int(os.environ.get("SOCAI_OBSERVER_FORCE_CAPTURE_INTERVAL", "300"))
            ),
            diff_threshold=min(
                0.95,
                max(0.01, float(os.environ.get("SOCAI_OBSERVER_DIFF_THRESHOLD", "0.30"))),
            ),
            screenshot_strategy=os.environ.get(
                "SOCAI_OBSERVER_SCREENSHOT_STRATEGY", "app_switch"
            ).strip() or "app_switch",
            capture_backend=_capture_backend(
                os.environ.get(
                    "SOCAI_OBSERVER_CAPTURE_BACKEND",
                    OBSERVER_CAPTURE_BACKEND_DEFAULT,
                )
            ),
            capture_all_displays=_env_flag("SOCAI_OBSERVER_CAPTURE_ALL_DISPLAYS", True),
            enable_visual_summary=_env_flag("SOCAI_OBSERVER_VISION_ENABLED", True),
            vision_model=os.environ.get(
                "SOCAI_OBSERVER_VISION_MODEL", OBSERVER_VISION_MODEL
            ).strip()
...
class ObserverCaptureService:
    ...
    def _save_capture_image(
        ...
        *,
        ...
        fd, temp_raw = tempfile.mkstemp(prefix="socai_observer_", suffix=".png")
        os.close(fd)
    ...
    def _capture_display_with_screencapture(self, display) -> Image.Image:
        ...
        for attempt in range(2):
            fd, temp_raw = tempfile.mkstemp(prefix=f"socai_display_{display.index}_", suffix=".png")
            os.close(fd)
            temp_path = Path(temp_raw)
            try:
                try:
...
def install_launch_agent(paths: ObserverPaths) -> Path:
    ...
    environment = {
        ...
        "SOCAI_OBSERVER_ROOT": str(paths.root),
    }
    for key in (
