...
(``socai auth`` with no subcommand) that guides users through credential
...
def build_parser() -> argparse.ArgumentParser:
    ...
    set_parser = subparsers.add_parser("set", help="Persist a credential into ~/.socai/auth.json.")
    ...
    clear_parser = subparsers.add_parser("clear", help="Remove a stored credential from ~/.socai/auth.json.")
...
def _run_login(provider: str, method: str) -> int:
    ...
    if provider == PROVIDER_OPENAI and method == METHOD_API_KEY:
        print("Use `printenv OPENAI_API_KEY | codex login --with-api-key` or `socai auth set openai api_key`.")
        return 0
    ...
    if config is not None:
        ...
        print(
            ...
            f"Use `socai auth set {provider} api_key` or set ${hint}."
        )
        return 0
