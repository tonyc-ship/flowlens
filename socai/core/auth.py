...
SOCAI_DIR = Path.home() / ".socai"
SOCAI_AUTH_FILE = SOCAI_DIR / "auth.json"
...
# ---------------------------------------------------------------------------
# socai/auth.json storage helpers
# ---------------------------------------------------------------------------
...
def _socai_auth_config() -> dict:
    if not SOCAI_AUTH_FILE.exists():
        return {}
    data = _read_json(SOCAI_AUTH_FILE)
    return data if isinstance(data, dict) else {}


def _write_socai_auth_config(data: dict) -> Path:
    SOCAI_DIR.mkdir(parents=True, exist_ok=True)
    SOCAI_AUTH_FILE.write_text(
...
def save_auth_secret(provider: str, method: str, secret: str) -> Path:
    ...
    return _write_socai_auth_config(data)
...
def _config_secret(provider: str, key: str) -> str:
    block = _socai_auth_config().get(provider) or {}
...
def discover_credential(
    ...
    *,
    ...
    socai_auth_source = str(SOCAI_AUTH_FILE.expanduser())

    if method == METHOD_API_KEY:
        for env_name in config.api_key_env:
            env_sources.append((env_name, os.environ.get(env_name, "")))
        config_sources.append((socai_auth_source, _config_secret(provider, "api_key")))
        ...
        for env_name in config.auth_token_env:
            env_sources.append((env_name, os.environ.get(env_name, "")))
        config_sources.append((socai_auth_source, _config_secret(provider, "auth_token")))
        if provider == PROVIDER_OPENAI:
...
def preferred_provider() -> str | None:
    block = _socai_auth_config().get("defaults") or {}
...
def default_model_for_provider(provider: str) -> str:
    ...
    defaults_block = _socai_auth_config().get("defaults") or {}
