...
from .runtime import socai_state_root
...
def _bridge_lock_path(port: int) -> Path:
    return socai_state_root() / "state" / f"bridge_{port}.lock"
...
class ExtensionBridge:
    ...
    async def ensure_extension_current(self) -> None:
        ...
        if os.environ.get("SOCAI_EXTENSION_AUTO_RELOAD", "1").lower() in {"0", "false", "no"}:
