"""Xiaohongshu CDP diagnostics for the Socai desktop runtime.

This module is intentionally site-specific. Generic CDP primitives live in
``socai.cdp``; this file only describes how to probe XHS page reachability,
...
from socai.cdp.diagnostics import SOCAI_TITLE_PREFIX
from socai.cdp.discovery import INSPECT_URL
from socai.cdp.errors import CDPDependencyError, add_chrome_permission_hint, exception_message
from socai.cdp.page import CDPPage, create_page_target
from socai.cdp.session import ExistingChromeConfig, connect_existing_chrome
...
async def run_xhs_cdp_probe(config: XHSCdpProbeConfig | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        await page.set_title_prefix(f"{SOCAI_TITLE_PREFIX} — XHS")
        await page.navigate(config.url, wait_seconds=config.load_wait)
        marked_title = await page.set_title_prefix(f"{SOCAI_TITLE_PREFIX} — XHS")
...
def classify_xhs_probe(diagnostics: dict[str, Any], after_screenshot: Path) -> tuple[str, str]:
    ...
    if login_required:
        return "xhs_login_required", "Opened Xiaohongshu in a Socai-controlled tab, but the user still needs to log in."
    return "xhs_probe_ready", "Opened Xiaohongshu in a Socai-controlled Chrome tab and confirmed the page is reachable."
...
def parse_xhs_probe_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Open Xiaohongshu in a Socai-controlled Chrome CDP tab.")
